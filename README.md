...
This pipeline was not our first attempt. We iterated through four methodological versions, each eliminating a specific confound:

| Version      | Method                                             | Confound Present                             | Hindi AUC        |
| ------------ | -------------------------------------------------- | -------------------------------------------- | ---------------- |
| v1           | Natural completion vs Wikipedia text               | Source-distribution (Wikipedia vs LLM style) | 0.8183           |
| v2           | Adversarial instruction + leakage cleanup          | Source-distribution                          | 0.9537           |
| v3           | Double-Generation (LLM writes both truth and lie)  | Prompt-type (model knows it was told to lie) | 0.8931           |
| **v4** | **Natural Generation + Semantic Similarity** | **None**                               | **0.9163** |

Each version's failure mode informed the design of the next. The full technical documentation of each pivot (including the specific bugs discovered) is available in `updates.md`.
...
| ---- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | `generate_data.py`         | Extracts Hindi Wikipedia sentences, truncates them, and prompts Llama-3.2-3B to complete naturally. Streams output to JSONL with automatic resume on interruption.              |
...
| 3    | `extract_hidden_states.py` | Passes labeled text through frozen Llama-3.2-3B with `output_hidden_states=True`. Extracts two MIND-style feature vectors per sample (6,144 dims total).                      |
| 4    | `train_probe.py`           | Trains a 3-layer MLP probe on the extracted hidden states. Uses `BCEWithLogitsLoss(pos_weight=2.0)` to prioritize hallucination recall. Reports Test AUC on a held-out split. |
...
Built as part of the Advanced NLP course (Semester VI) at Plaksha University.

- **Sandeep** — Pipeline architecture, model configuration, probe training, code implementation
- **Ramam** — Literature review, dataset sourcing, methodology design
- **Anant** — Evaluation protocol, BHRAM-IL analysis, debugging
