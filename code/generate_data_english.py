...
This script serves as our English baseline to replicate the original MIND 
framework logic. We use it to ensure the unexpectedly high or low AUC 
metrics aren't just an artifact of our pipeline implementation.

Usage:
    # Run both modes separately to get clean English baselines
    python code/generate_data_english.py --num_samples 500 --mode natural
...
def get_english_wikipedia_sentences(num_sentences=1000):
    ...
    for article in tqdm(dataset, desc="Extracting sentences", total=num_sentences * 3):
        text = article["text"]
        # Split on English period '.'
        # basic heuristic: split by '. ' to avoid decimal numbers
...
# List of English instruction prefixes to strip
LEAKAGE_PATTERNS = [
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are a helpful assistant. I will give you a partial sentence and its original "
                "full version. You must complete the partial sentence naturally in English using the information "
                "from the original sentence. Do NOT copy word-for-word if possible, but stay factually correct. "
                "Only output the completion, nothing else."
            ),
        },
        {
            "role": "user",
            "content": f"Partial: {partial_sentence}\nFull sentence: {full_sentence}\nCompletion:",
        },
    ]
...
def generate_completion(model, tokenizer, partial_sentence, mode="adversarial"):
    if mode == "natural":
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"Complete this sentence with a factually incorrect but plausible ending: {partial_sentence}",
            },
        ]
...
def create_double_pairs(model, tokenizer, sentences, truncation_range=(0.4, 0.6)):
    """Generate both truth and lie using the LLM for each sentence (Double-Generation)."""
    pairs = []
    skipped = 0
    retried = 0
    ...
    for sent in tqdm(sentences, desc="Generating double pairs (English)"):
        ...
        # 1. Generate Truth (LLM rephrasing)
        llm_truth = ""
        for attempt in range(MAX_RETRIES + 1):
            llm_truth = generate_truth_completion(model, tokenizer, partial, sent)
            if len(llm_truth.split()) >= 3: break
            if attempt < MAX_RETRIES: retried += 1

        # 2. Generate Lie (Adversarial)
        llm_lie = ""
        for attempt in range(MAX_RETRIES + 1):
            llm_lie = generate_completion(model, tokenizer, partial, mode="adversarial")
            if len(llm_lie.split()) >= 3: break
            if attempt < MAX_RETRIES: retried += 1
    ...
    if skipped > 0: print(f"Skipped {skipped} pairs due to empty/short completions")
    if retried > 0: print(f"Retried {retried} completions due to instruction leakage")
    return pairs


def create_pairs(model, tokenizer, sentences, mode="adversarial", truncation_range=(0.4, 0.6)):
    ...
    for sent in tqdm(sentences, desc=f"Generating pairs ({mode} mode)"):
        ...
        for attempt in range(MAX_RETRIES + 1):
            ...
            if len(llm_completion.split()) >= 3: break
            if attempt < MAX_RETRIES: retried += 1
...
def create_semantic_samples(model, tokenizer, sentences, output_file, truncation_range=(0.4, 0.6), start_idx=0):
    """Generate UNLABELED natural completions for semantic similarity labeling (English)."""
    samples = []
    skipped = 0
    retried = 0
    ...
    with open(output_file, "a", encoding="utf-8") as f:
        for sent in tqdm(sentences_to_process, desc="Generating natural completions", initial=start_idx, total=start_idx+len(sentences_to_process)):
            words = sent.split()
            truncation_point = random.randint(
                int(len(words) * truncation_range[0]),
                int(len(words) * truncation_range[1]),
            ...
            llm_completion = ""
            for attempt in range(MAX_RETRIES + 1):
                llm_completion = generate_completion(model, tokenizer, partial, mode="natural")
                if len(llm_completion.split()) >= 3: break
                if attempt < MAX_RETRIES: retried += 1
    ...
    if skipped > 0: print(f"Skipped {skipped} samples due to empty/short completions")
    if retried > 0: print(f"Retried {retried} completions")
    return samples


def main():
    parser = argparse.ArgumentParser(description="Generate English baseline facts/hallucinations")
    ...
    print(f"\n{'='*60}\nLanguage: ENGLISH | Mode: {args.mode.upper()} | Samples: {args.num_samples}\n{'='*60}\n")
    ...
    if args.mode == "semantic":
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            for pair in pairs: f.write(json.dumps(pair, ensure_ascii=False) + "\n")
    else:
